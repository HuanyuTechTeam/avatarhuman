...
async function loadConfig() {
  try {
    const response = await fetch('/avatarhuman/langchain-s-assets/config.json');
...
let kb_name, model, score_threshold, prompt_name, Keywords, voice_id;
async function init() {
  await loadConfig();
  kb_name = appConfig.langchain.kb_name;
  model = appConfig.langchain.model;
  score_threshold = appConfig.langchain.score_threshold;
  prompt_name = appConfig.langchain.prompt_name;
  Keywords = appConfig.wakeWords;
  voice_id = appConfig.voice.defaultVoiceId;
  console.log("配置文件已加载:", kb_name, model, score_threshold, prompt_name, voice_id, Keywords);
}
// 启动初始化
