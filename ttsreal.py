...
class indexTTS(BaseTTS):
    ...
    def index_tts(self, text, reffile, server_url) -> Iterator[bytes]:
        ...
        try:
            ...
            logger.info(f"index_tts Time to make POST: {end-start}s")
            ...
            for chunk in res.iter_content(chunk_size=None): #12800 1280 32K*20ms*2
                ...
                if first:
                    ...
                    logger.info(f"index_tts Time to first chunk: {end-start}s")
                    first = False
                if chunk and self.state==State.RUNNING:
                    yield chunk
            #print("index_tts response.elapsed:", res.elapsed)
        except Exception as e:
            logger.exception('indextts')
